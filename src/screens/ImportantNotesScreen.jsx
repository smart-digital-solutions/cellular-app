...
// Theme configuration for different severities
const SEVERITY_THEMES = {
  ...
  info: {
    wrapper: 'bg-indigo-500/10 border-indigo-500/30',
  ...
  return (
    <div className="animate-in fade-in max-w-4xl mx-auto z-10 relative">
      <div className="text-center mb-10 pt-4">
        ...
        <p className="text-sm font-medium max-w-xl mx-auto text-slate-400">
      ...
      <div className="grid md:grid-cols-2 gap-6 relative">
        {sortedNotes.map((note) => {
          ...
          return (
            <div 
              key={note.id} 
              className={`rounded-2xl p-6 border transition-all duration-300 hover:shadow-lg ${theme.wrapper} bg-[#0B1120]`}
            >
        ...
        {sortedNotes.length === 0 && (
          <div className="col-span-full text-center py-10 opacity-50 bg-[#0B1120] border border-white/10 rounded-2xl">
            <AlertTriangle className="w-12 h-12 mx-auto mb-3 opacity-20 text-white" />
            <p className="font-bold text-white">אין דגשים חשובים כרגע</p>
          </div>
        )}
      </div>
    </div>
