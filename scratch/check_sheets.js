...
async function run() {
  try {
    const data = await fetchSheet('guide');
    console.log('GUIDE SHEET CONTENT LENGTH:', data.length);
    fs.writeFileSync('scratch/guide_sheet_data.json', JSON.stringify(data, null, 2));
    console.log('Wrote sheet data to scratch/guide_sheet_data.json');
  } catch (err) {
    console.error(err);
  }
}
